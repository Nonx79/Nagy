...
public class IA : MonoBehaviour
{
    ...
    void Path()
    {
        StartCoroutine(map.selectedUnit.GetComponent<Unit>().moveOverSeconds(map.selectedUnit, map.selectedUnit.GetComponent<Unit>().currentPath[map.selectedUnit.GetComponent<Unit>().currentPath.Count - 1]));
        StartCoroutine(moveUnitAndFinalize());                  
    }
    ...
    void GeneratePathTo(int x, int y)
    {
        ...
        remMove = map.selectedUnit.GetComponent<Unit>().moveSpeed;
    }
    ...
    public IEnumerator moveOverSeconds(GameObject objectToMove, Node endNode)
    {
        ...
        map.selectedUnit.GetComponent<Unit>().currentPath.RemoveAt(0);
        while (map.selectedUnit.GetComponent<Unit>().currentPath.Count != 0)
        {
            Vector3 endPos = map.TileCoordToWorldCoord(map.selectedUnit.GetComponent<Unit>().currentPath[0].x, map.selectedUnit.GetComponent<Unit>().currentPath[0].y);
            objectToMove.transform.position = Vector3.Lerp(transform.position, endPos, map.selectedUnit.GetComponent<Unit>().visualMovementSpeed);
        ...
        map.tilesOnMap[map.selectedUnit.GetComponent<Unit>().unitPreviousX, map.selectedUnit.GetComponent<Unit>().unitPreviousY].GetComponent<ClickableTile>().unitOnTile = null;
        map.tilesOnMap[map.selectedUnit.GetComponent<Unit>().tileX, map.selectedUnit.GetComponent<Unit>().tileY].GetComponent<ClickableTile>().unitOnTile = this.gameObject;
